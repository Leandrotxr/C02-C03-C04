...
int shortest_path_dijkstra(Cidade adj[], int vertices, int start, int end, float idh){
    ...
    while(intree[v] == false){
    	...
    	for(p = adj[v].vizinhos.begin(); p != adj[v].vizinhos.end(); p++){
            int dest = p->dest;
            int weight = p->distancia;
            if(distance[dest] > distance[v]+weight && adj[v].idh >= idh){
