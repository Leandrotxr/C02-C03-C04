...
void show_ordered(Node * curr, int ValorMin, int ValorMax)
{
	if(curr->left != NULL)
		show_ordered(curr->left, ValorMin, ValorMax);
	if(curr->data.key >= ValorMin && curr->data.key <= ValorMax)
	{
	...
	{
		remove(curr, curr->data.value);
	}
	if(curr->right != NULL)
		show_ordered(curr->right, ValorMin, ValorMax);
}
